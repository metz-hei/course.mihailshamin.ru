...
16. WIP – количество задач одновременно находящихся в работе. Разделяется по разным стадиям работы над задачей.
17. Lead Time – время от появления задачи до ее закрытия. Включает Cycle Time и время ожидания в очереди на реализацию.
18. Cycle Time – время от начала работ по задаче до их завершения.
19. Wasted Time – время, которое задача проводит в различных очередях, а не непосредственно в работе.
20. Review Time — время, затраченное на Code Review.
21. Test Time — время, затраченное на тестирование.
22. Effectiveness – процент времени, которое тратится непосредственно на работу с задачей, а не на ожидания в различных очередях.
23. Throughput – количество задач, которое может выполнять команда в единицу времени (день, неделя, месяц).
